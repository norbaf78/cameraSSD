...
if __name__ == '__main__':
    ...
    #################################################### 
    resize_img = 3 #1.2 # the resize to the acquired image. The HOG will be evaluated on the resized image
    ...
    # where is not possible that peaple ore detected (outside a valid region. This type of error happend if the bottom
    # side of the rectangle that identify the person is for some reason on the wall and for this reason is reprojected outside the room)
    #################################################### 
    ...
    #cap=cv2.VideoCapture("http://192.168.208.202/image.mjpg")
    #cap=cv2.VideoCapture("http://admin:admin@192.168.208.200/jpg/image.jpg?size=3")
    
    #cap=cv2.VideoCapture("rtsp://root:progtrl01@192.168.208.200/axis-media/media.amp")
    ...
    while True:
        ...
        for (objectID, centroid) in objects.items():
            ...
            idAnonymous = "anonymous_" + str(objectID)
        ...
        # polygon identify the valid region. The polygon that for every reason are detected outside this 
        # this region are moved in the closest point inside this region
        cv2.line(img_map, valid_polygon[0], valid_polygon[1], (0,255,0), 4)
        cv2.line(img_map, valid_polygon[1], valid_polygon[2], (0,255,0), 4)
        cv2.line(img_map, valid_polygon[2], valid_polygon[3], (0,255,0), 4)
        cv2.line(img_map, valid_polygon[3], valid_polygon[0], (0,255,0), 4)
